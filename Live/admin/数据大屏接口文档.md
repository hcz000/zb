...
> **✅ 已实现，直接使用**（后端：`backend/app/routers/live.py`，已注册到 `/api` 前缀；前端 `admin-api.js` 实际调用的是 `/api/v1/admin/` 别名）
>
> 所有直播控制接口均支持 `stream_id`（开始/停止时可省略，自动使用当前活动流）。

- `POST /api/v1/admin/live/start` - 开始直播
  - 请求体：`{ "streamId": "stream-xxx", "autoStartAI": false, "notifyUsers": true }`（均可选）
  - 返回：`{ liveId, streamUrl, status: "started", streamId, startTime, notifiedUsers }`
  - 广播：`liveStatus`（`isLive: true`）；若 `autoStartAI` 为 `true` 额外广播 `aiStatus`（`status: "running"`）
- `POST /api/v1/admin/live/stop` - 停止直播
  - 请求体：`{ "streamId": "stream-xxx", "saveStatistics": true, "notifyUsers": true }`（均可选）
  - 返回：`{ liveId, status: "stopped", streamId, stopTime, duration, summary, notifiedUsers }`
  - 广播：`liveStatus`（`isLive: false`）
- `POST /api/admin/live/control` - 简单控制（action: `start` / `stop`）
- `POST /api/admin/live/setup-and-start` - 立即开播或设置定时开播
  - 请求体：`{ "streamId", "startNow": true }` 或 `{ "scheduledStartTime", "scheduledEndTime", "streamId" }`
- `GET /api/v1/admin/live/status` - 获取直播状态（是否开播、当前活动流、直播计划）
- `POST /api/admin/live/schedule` / `GET /api/admin/live/schedule` / `POST /api/admin/live/schedule/cancel` - 设置 / 查询 / 取消直播计划
  - 设置/取消时广播：`live-schedule-updated` / `live-schedule-cancelled`
- `GET /api/v1/admin/live/viewers?stream_id=xxx` - 获取观看人数（按流或全局）
- `POST /api/v1/admin/live/broadcast-viewers` - 主动广播观看人数（广播事件：`viewers-updated`）

> **前端使用位置：** `admin-api.js` 的 `startLive()` / `stopLive()` 分别调用 `/api/v1/admin/live/start`、`/api/v1/admin/live/stop`；后台管理页 `index.html` 的「开始直播 / 停止直播」按钮即触发这两个接口。
...
数据大屏通过WebSocket接收实时更新。WebSocket连接地址：与页面同源的 `/ws`（经 `live-gateway` 由 `http://localhost:8080` 代理转发到后端，因此实际为 `ws://localhost:8080/ws`）
...
- `live-status-changed`：`{ status: "started" | "stopped", streamId, streamUrl, timestamp }`
- `viewers-updated`：`{ streamId, viewers }`（观看人数更新，由 `broadcast-viewers` 触发）
- `live-schedule-updated` / `live-schedule-cancelled`：直播计划变更（由 `schedule` / `schedule/cancel` 触发）
- `aiStatus`：`{ status: "running" | "stopped", streamId, aiSessionId }`（AI 自动点评状态，开播时可由 `autoStartAI` 触发）

> 前端 `admin-events.js` 监听 `liveStatus` 事件，用于实时切换「开始直播 / 停止直播」按钮的可用状态与顶部「🟢 直播中 / ⚪ 未开播」标识。
...
> **✅ 已实现，直接使用**（后端：`backend/app/routers/auth.py`，已注册到 `/api` 前缀并包含 `/api/v1/admin/` 别名；前端 `admin-api.js` 的 `getUsers()` 调用分页接口 `/api/admin/miniprogram/users`）

#### 微信小程序登录
- `POST /api/wechat-login` - 微信登录
  - 请求体：`{ "code": "wx_login_code", "userInfo": { "nickName": "...", "avatarUrl": "..." } }`
  - 返回：`{ openid, session_key, unionid, userInfo, loginTime, isMock }`
  - 说明：服务首次登录时自动创建/更新用户记录；当前默认使用 mock 模式（无需真实微信 code）

#### 用户列表
- `GET /api/admin/users` - 获取全部用户列表（基础，用于评委管理页面选择评委；v1 别名：`GET /api/v1/admin/users`）
- `GET /api/admin/users/{user_id}` - 获取单个用户详情
- `GET /api/admin/miniprogram/users?page=1&pageSize=20&status=all&orderBy=joinTime` - 分页获取小程序用户列表
  - 排序：`orderBy=joinTime`（按加入时间）或 `orderBy=votes`（按投票数降序）
  - 返回：`{ total, page, pageSize, users: [{ userId, nickname, avatar, status, lastActiveTime, statistics: { totalVotes, totalComments, totalLikes, currentPosition }, joinTime }] }`

> **前端使用位置：** `admin-api.js` 的 `getUsers(page, ...)` 调用 `/api/admin/miniprogram/users` 实现后台用户管理列表的翻页和排序。
...
- [x] ✅ **前端实现：`startSegment(segmentIndex)` 函数** - 用于从指定环节开始计时（已实现）
- [x] ✅ **前端确认：`prevSegment()` 函数名** - 已使用 `prevSegment()` 函数名（与 `nextSegment()` 对应）
- [x] 票数趋势图表显示（按流独立记录历史）
- [x] 评委投票对抗条显示（按流独立）
...
4. ✅ **前端已实现函数** - `startSegment(segmentIndex)` 函数已在 `Live/viewer/index.html` 中实现
5. ✅ **前端函数名统一** - 使用 `prevSegment()` 与 `nextSegment()` 保持命名一致
