...
> 微信小程序端为 Mock 登录，输入任意符合格式的模拟 `code` 即可登录，系统内置 8 个演示用户。
...
- 投票/评委/辩论流程数据均为 Mock 进程内存存储，重启即丢失，符合需求"使用 Mock 数据"的要求。
- 数据大屏（`viewer/index.html`）已完整支持 streamId 过滤、真实计时器、评委卡片、辩论流程实时展示，不再兜底使用默认数据。
